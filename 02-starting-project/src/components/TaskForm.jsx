    ...
    return (
        <form onSubmit={(e) => handleAddTask(e, projectId)} className="p-[25px] mt-[25px] border border-black">
            <div className="flex flex-row gap-4 justify-end">
                <button type="button" onClick={handleClickCancelAddTask} className="text-stone-700 hover:text-stone-950">
                    Cancel
                </button>
            ...
            <div className="flex flex-row gap-4 justify-start">
                <div className="flex flex-col gap-4">
                    ...
                    <input onChange={(e) => handleOnChangeTaskInput(e.target.value)} id="title" type="text" required className="w-64 px-2 py-1 rounded-sm bg-stone-200" />
                </div>
            </div>
        </form>
    );
